...
class TeamTest(commands.Cog):
    ...
    async def delete(self, ctx):
        ...
        if role:
            try:
                await role.delete()
                await ctx.send(f"Deleted {role.name} role!")
            except discord.Forbidden:
                await ctx.send("I need Manage Roles permission!")
            except discord.HTTPException:
    ...
    # TEAM MEMBER COMMANDS - OWNER AND TEAM MEMBERS ONLY
    @team.command()
    @commands.check_any(commands.is_owner(), commands.check(lambda ctx: ctx.cog.team_member_check(ctx)))
